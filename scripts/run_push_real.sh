...
python editing_structure.py \
    --prompt "A photo of a sks red cube and a gripper on a white table" \
    --image_path "./data/example_images/push_real/$i.png" \
    --experiment_name "push_real_$i" \
    --region 0.1 0.3 0.7 0.9 \
    --noise_level 0.5 \
    --prompt_indice 5 6 7 \
    --num_trailing_maps 30 \
    --editing_steps 10 \
    --annealing_coef 0.01 \
